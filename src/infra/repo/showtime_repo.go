...
func (r *ShowtimeRepo) Create(ctx context.Context, showtime *domain.Showtime) (*domain.Showtime, error) {
...
func (r *ShowtimeRepo) GetListByFilter(ctx context.Context, movieId uuid.UUID, cinemaId uuid.UUID, day time.Time) ([]*domain.Showtime, error) {
...
func (r *ShowtimeRepo) FindConflictByRoomId(ctx context.Context, roomId uuid.UUID, startTime, endTime time.Time) ([]domain.Showtime, error) {
	var conflictingShowtimes []domain.Showtime
	if err := r.db.WithContext(ctx).Preload("Movie").Where("room_id = ? AND start_time < ? AND end_time > ?", roomId, endTime, startTime).
...
func (r *ShowtimeRepo) GetListByCinemaFilter(ctx context.Context, id uuid.UUID, day time.Time) ([]*domain.Showtime, error) {
	var showtimes []*domain.Showtime
...
func (r *ShowtimeRepo) GetListByRoomFilter(ctx context.Context, id uuid.UUID, day time.Time) ([]*domain.Showtime, error) {
	var showtimes []*domain.Showtime
...
func (r *ShowtimeRepo) GetById(ctx context.Context, id uuid.UUID) (*domain.Showtime, error) {
	...
	if err := r.db.WithContext(ctx).Preload("Tickets").Preload("Room").Where("id = ?", id).First(&showtime).Error; err != nil {
		return nil, r.returnError(ctx, err)
	}
	return &showtime, nil
}
...
func (r *ShowtimeRepo) FindConflictToUpdate(ctx context.Context, roomId uuid.UUID, startTime, endTime time.Time, id uuid.UUID) ([]domain.Showtime, error) {
	var conflictingShowtimes []domain.Showtime
	if err := r.db.WithContext(ctx).Preload("Movie").Where("id != ? AND room_id = ? AND start_time < ? AND end_time > ?", id, roomId, endTime, startTime).
		Find(&conflictingShowtimes).Error; err != nil {
		return nil, r.returnError(ctx, err)
	}
	return conflictingShowtimes, nil
...
func (r *ShowtimeRepo) GetByMovieIDAndDateRange(ctx context.Context, movieID uuid.UUID, startDate, endDate time.Time) ([]domain.Showtime, error) {
	var showtimes []domain.Showtime
	if err := r.db.WithContext(ctx).Where("movie_id = ? AND start_time BETWEEN ? AND ?", movieID, startDate, endDate).Find(&showtimes).Error; err != nil {
		return nil, r.returnError(ctx, err)
	}
	return showtimes, nil
}

func (r *ShowtimeRepo) GetByRoomIDAndDateRange(ctx context.Context, roomID uuid.UUID, startDate, endDate time.Time) ([]domain.Showtime, error) {
	var showtimes []domain.Showtime
	if err := r.db.WithContext(ctx).Where("room_id = ? AND start_time BETWEEN ? AND ?", roomID, startDate, endDate).Find(&showtimes).Error; err != nil {
		return nil, r.returnError(ctx, err)
	}
	return showtimes, nil
}
